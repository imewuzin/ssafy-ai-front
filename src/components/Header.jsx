...
export default function Header({ logo, title }) {
    ...
    return (
        <header className="bg-[#F3FBFE] text-black shadow-sm p-4 flex items-center justify-between">
            <div className="flex items-center">
                {logo && <img src={logo} alt="SSAFY Logo" className="h-12 mr-3" />}
                <h1 className="text-2xl font-bold">{title}</h1>
            </div>
            <button className="rounded-full hover:bg-[#00B3E3] transition-colors p-2" onClick={openModal}>
                ...
                <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white p-6 rounded-lg max-w-md shadow-lg">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={closeModal} className="text-gray-500 hover:text-[#1428A0]">
                                <X className="w-6 h-6"/>
                            </button>
                        </div>
                        ...
                        <p className="text-sm opacity-90">2024.01.03 SSAFY 계절학기 AI 코스 서울 5팀</p>
